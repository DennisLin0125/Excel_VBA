...
'*****************************************************************************************************************
        ...
        MeetAddress = "P:\Service\Service meeting\" & myYear & "\" & myTime2 & "\" & "RMA Weekly Status" & myTime2 & ".xls"  '開啟Weekly
        meetAns = fs.FileExists(MeetAddress)
        
        RPAddress = "P:\Service\Service meeting\" & myYear & "\" & myTime2 & "\" & "Utilization" & myTime2 & ".xls"     '開啟工時
        RPAns = fs.FileExists(RPAddress)
        
        RPNumAddress = "P:\Service\Service meeting\" & myYear & "\" & myTime2 & "\" & "維修台數" & myTime2 & ".xls"     '開啟維修台數
        RPNumAns = fs.FileExists(RPAddress)
        
        CpleRateAddress = "P:\Service\Service meeting\" & myYear & "\" & myTime2 & "\" & myYear & "." & myMonth & "." & myDay & " 達成率.xlsx"     '開啟達成率
...
'                        workTime = InputBox("請輸入James資料", "輸入")
'                        Cells(i, ActiveCell.Column) = workTime
                End If
        Next
        
'        Worksheets("新品開發").Select
'
'        For i = 260 To 1000
